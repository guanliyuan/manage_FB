    ...
    var calendar = function(calendarId){
        ...
        c.init = function(){
            ...
            * .redTr  默认的背景色*/
            ...
            if(loadStyles.mark != 'load'){
                loadStyles(".calendarDiv {width:100%;text-align:center;}\
            .Table{width:100%;height:300px;margin-top: 50px;}\
            .box{text-align:center;width:160px;height:22px;float:right;margin-right:65px;margin-top:-100px;font-weight:bold;font-size:21px;color:#8d8d8d;}\
            ...
            .Tr{color:#d9d9d9;text-align:center;}\
            .redTr{color:#ffffff;text-align:center;width: 30px;height: 30px;border-radius: 50%;background-color: #1a87ec;border: 1px solid #1a87ec;}\
            .blueTr{color:#8d8d8d;text-align:center;cursor: pointer;};\
            .weekArr{color:#8d8d8d;}");
            }
            ...
            c.buildPart({
                node : 'span',
                id : 'Year',
                innerHTML : thisyear + '-',//innerHTML : thisyear + '年',
                appendToBox : headId
            });
            //生成存储月份的span
            c.buildPart({
                node : 'span',
                id : 'Month',
                innerHTML : thismonth < 10 ? '0'+ thismonth: thismonth,//innerHTML : thismonth < 10 ? '0'+ thismonth + '月' : thismonth,
                appendToBox : headId
            });
            //生成存储增加月份的div
            c.buildPart({
        ...
        (function(){
            ...
            dec.onclick = function(e){
                ...
                if(target.id == 'dec'){
                    if(month>1 || month== 12){
                        Month.innerHTML = --month < 10 ? '0' + month: month;//--month < 10 ? '0' + month+ '月': month+ '月'
                        CalendarTr = c.getcalendar(year,month);
                        Tr.innerHTML = '';
                        Tr.innerHTML = CalendarTr;
                    }else{
                        Year.innerHTML  = --year <=1970 ? 1970 + '-' : year+'-';//--year <=1970 ? 1970 + '年' : year + '年'
                        Month.innerHTML = year < 1970 && month < 2 ? '01': 12;//year < 1970 && month < 2 ? '01月': 12 + '月'
                        //小于1970的时间显示为1970的时间
            ...
            add.onclick = function(e){
                ...
                if(target.id == 'add'){
                    if(month>0 && month<12){
                        ...
                        Year.innerHTML  = ++year+'-';//++year + '年'
                        Month.innerHTML = '0'+1;//'0'+1 + '月'
                        month = 1;
                        CalendarTr = c.getcalendar(year,month);
                        Tr.innerHTML = '';
                        Tr.innerHTML = CalendarTr;
