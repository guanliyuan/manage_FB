...
(function () {
        ...
        init: function (a, d) {
            ...
            if ('string' == typeof(a)) {
                if (a.indexOf('#') == 0) {
                    ...
                    if (b) return b; else return null
                }
                var b = (d || document).getElementById(a);
                if (b) return g(b); else return null
            } else {
                this[0] = a;
                this.length = 1;
                return this
            ...
            if (g.ie) return this[0].currentStyle[p]; else return this[0].ownerDocument.defaultView.getComputedStyle(this[0], '').getPropertyValue(p)
        }, gtag: function (n) {
            ...
            if (g.ie) this[0].attachEvent('on' + n, f); else this[0].addEventListener(n, f, false);
            return this
        }, revt: function (n, f) {
            if (g.ie) this[0].detachEvent('on' + n, f); else this[0].removeEventListener(n, f, false);
            return this
        }, alnk: function (c) {
            if (g.ie) return this[0].createStyleSheet(c).owningElement; else {
    ...
    g.exend = g.ret.exend = function () {
        ...
        for (; i < length; i++) if ((options = arguments[i]) != null) for (var b in options) {
            ...
            if (a === copy) continue;
            if (deep && copy && typeof copy == 'object' && !copy.nodeType) a[b] = g.extend(deep, c || (copy.length != null ? [] : {}), copy); else if (copy !== undefined) a[b] = copy
        }
        return a
    };
    g.ret.exend({
        stopac: function (o) {
            ...
            if (f) this[0].onfocus = f; else this[0].focus();
            return this
        }, msdown: function (f) {
            ...
            if (f) this[0].onsubmit = f; else this[0].onsubmit();
            return this
        }, cmenu: function (f) {
    ...
    g.exend({
        build: '1.0.0', author: 'LiHuiGang', path: function (t) {
            t = t || 'lhgcore.js';
            ...
            if (g.ie) {
                ...
                if (doc && doc.clientWidth) b = doc; else b = a.document.body;
                if (b) return {w: b.clientWidth, h: b.clientHeight}; else return {w: 0, h: 0}
            } else return {w: a.innerWidth, h: a.innerHeight}
        }, spos: function (w) {
            if (g.ie) {
                var a = w.document;
                oPos = {x: a.documentElement.scrollLeft, y: a.documentElement.scrollTop};
                if (oPos.x > 0 || oPos.y > 0) return oPos;
                return {x: a.body.scrollLeft, y: a.body.scrollTop}
            } else return {x: w.pageXOffset, y: w.pageYOffset}
        }, dpos: function (w, n) {
            ...
            while (cn && !(cw == w && (cn == w.document.body || cn == w.document.documentElement))) {
                ...
                pn = cn;
                if (cn.offsetParent) cn = cn.offsetParent; else {
                    if (cw != w) {
                        cn = cw.frameElement;
                        pn = null;
            ...
            if (g.ie) {
                ...
                if (g.mz) o.style.MozUserSelect = 'none'; else if (g.sa) o.style.KhtmlUserSelect = 'none'; else o.style.userSelect = 'none'
            }
        }, gtvod: function () {
    ...
    g.exend({
        cleanup: function () {
            if (window._lhgcleanobj) this.citem = window._lhgcleanobj.citem; else {
                this.citem = [];
    ...
    J.exend({
        panel: function (b, w) {
            ...
            J(a).attr({src: 'javascript:void(0)', frameBorder: 0, scrolling: 'no'}).stcs({
                display: 'none',
                position: 'absolute',
                zIndex: 19700
            });
    ...
    J.exend(J.panel.prototype, {
        applnk: function (l) {
            ...
            J(this._ifrm).stcs({left: x + 'px', top: y + 'px', width: iw + 'px', height: ih + 'px'})
        }, hide: function (e, a) {
    ...
    g.ajax = g.A = {
        geth: function () {
            ...
            if (m == 'GET') b.send(null); else {
                b.setRequestHeader('content-type', 'application/x-www-form-urlencoded');
                if (p) b.send(p); else return false
            }
            if (!a) {
